...
namespace Seemplest.MsSql.UnitTests.DataAccess
{
    ...
    public class UpdateAsyncTest
    {
        ...
        [TestMethod]
        public async Task SimpleUpdateAsyncWorksWithTouchAllFieldsAndIdentity()
        {
            // --- Arrange
            var db = new SqlDatabase(DB_CONN);
            await db.ExecuteAsync(@"create table sample(Id int identity not null, Name varchar(50) null)");
            var record = new SampleRecordWithAutoPropsAndIdentity { Name = "First" };
            await db.InsertAsync(record);
        ...
        [TestMethod]
        public async Task SimpleUpdateAsyncWorksWithTouchAllFlagAndIdentity()
        {
            // --- Arrange
            var db = new SqlDatabase(DB_CONN);
            await db.ExecuteAsync(@"create table sample(Id int identity not null, Name varchar(50) null)");
        ...
        [TableName("sample")]
        class SampleRecordWithAutoPropsAndIdentity : DataRecord<SampleRecordWithAutoPropsAndIdentity>
        {
            [PrimaryKey]
            [AutoGenerated]
            public int Id { get; set; }
